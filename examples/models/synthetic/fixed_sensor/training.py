...
from model5 import SpatialRectifiedRegressor
...
def plot_debugging(tanh_out):
    ...
    #        legends.append(f'layer {i}')
    #plt.legend(legends)
    #plt.title('backprop distribution')
    #plt.show()
...
parameters = {
    ...
    "random_noise": False,
    "noise_scale": None,
    "hidden_size": 96*2, #128,
    "dropout": 0.0, #0.5,
    "num_epochs": 3500,
    "device": "cpu",
    "last_model": "saved_models/model_15.pt",
    "best_model": "saved_models/best_model_15.pt",
    "plot": "plots/training_15.png",
    "save_every": 100,
    "log_every": 100,
    "n_heads": 4**2
}
...
train_data = read_data("../fixed_train.csv")
val_data = read_data("../fixed_val.csv")
...
for name, param in model.named_parameters():
    if param.ndim == 2:
        param.data *= (1/2) ** 2 #5/3 #(2/(1 + 0.25 ** 2)) ** (1/2) # 2 ** (1/2) #5/3 # gain
...
for epoch_index in range(parameters["num_epochs"]):
    ...
    for batch_index, batch_dict in enumerate(train_batch_generator):
        ...
        with torch.no_grad():
            ...
            grad_std.append([p.grad.std().item() for i, p in enumerate(model.parameters()) if p.data.ndim==2 and i != 9])


    if epoch_index % 500 == 0 and epoch_index != 0:
        plot_debugging([model.x_main, model.x_main_1, model.x_main_2, model.x_attn, model.x_attn_1, model.x0, model.x_1, model.x_2])
        #print(loss)
