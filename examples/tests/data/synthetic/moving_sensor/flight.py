...
def generate_sensor_data():
    ...
    df = pd.DataFrame({
        ...
        'lat': np.linspace(start_position[0], end_position[0], number_of_measuments), # sensor moves uniformly form start to end position
...
evaluation_set['y'] = np.sin(evaluation_set['t']) * evaluation_set['lat'] + np.cos(evaluation_set['t']) * evaluation_set['lon']
