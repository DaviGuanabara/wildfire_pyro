...
class PointNeighborhood(Dataset):
    def __init__(self,
        ...
        if normalize_time_difference is not None:
        ...
        if self.train and self.random_noise:
            print(
                f"[INFO] applying normal random noise of std dev {self.noise_scale} to input")
    ...
    def __getitem__(self, index):
        ...
        n_neighbors = 0
        # check if there are enought points in the past, else sample a new
        # point at random
        while n_neighbors < self.min_neighbors:
            ...
            older_points = self.data[np.where((self.data[:, 0] < t_i) & (
                np.abs(self.data[:, 0] - t_i) <= self.max_timestep_difference))]
            ...
            if n_neighbors >= self.max_neighbors:
                ...
                neighbors = np.random.randint(
                    low=self.min_neighbors, high=self.max_neighbors, size=1)
            ...
            if self.train and self.random_noise:
                u_j = np.array([[(point_j[:, 0] - point_i[0, 0]) / self.normalize_time_difference,
                                 (point_j[:, 1] - point_i[0, 1] +
                                  np.random.normal(0.0, self.noise_scale)),
        ...
        # y_i = data_point[3].reshape(1,) # in this case never normalizing
        # outputs
        y_i = data_point[3].reshape(1,) + 1.0  # summing 1.0 because of the log
        y_i = np.log(y_i)
    ...
    def create_data_point(self, u_j, neighbors):
        ...
        if self.max_neighbors > neighbors:
            ...
            mask = torch.cat(
                (ones, torch.zeros((self.max_neighbors - int(neighbors)), self.hidden)))
            u_j_t = torch.cat(
                (u_j_t, torch.zeros((self.max_neighbors - int(neighbors)), 4)))
