...
def predict_knn(features_to_predict, train_features, train_labels, n_samples=600, prob=0.7, n_neighbors=4):
...
if nn:
    parameters = {
        ...
        "best_model": "/Users/leonardo/wildfire-sinusoidal/taxi_nn_flight/saved_models/best_model_1.pt", #"/Users/leonardo/taxi-bj-data/taxi-flight/models/best_model_4_tanh.pt",
...
for index, i in enumerate(last_25_days):
    ...
    if nn:
        ...
        if len(test_data) > 60:
            ...
            for location in loc:    
                y_hat, y_conf = predict(loc=location, t=max_time, n_estimations=1600)
            ...
            plt.figure()
            plt.imshow(np.reshape(np.array(y_pred_list), (50,50)), vmin=min_limit, vmax=max_limit, cmap='RdYlGn_r') # (1, 50, 50, 1)
            plt.colorbar()
            ...
            plt.figure()
            plt.imshow(np.reshape(np.array(y_conf_list), (50,50)), vmin=min_limit, vmax=max_limit, cmap='Reds') # (1, 50, 50, 1)
            plt.colorbar()
            ...
            plt.figure()
            plt.imshow(np.zeros((50,50)), vmin=min_limit, vmax=max_limit, cmap='RdYlGn_r') # (1, 50, 50, 1)
            plt.colorbar()
            plt.savefig(f'video-images/predictions/prediction-{index}.png')
            plt.xlim([0, 50])
            plt.ylim([0, 50])
            ...
            plt.figure()
            plt.imshow(np.zeros((50,50)), vmin=min_limit, vmax=max_limit, cmap='Reds') # (1, 50, 50, 1)
            plt.colorbar()
            plt.savefig(f'video-images/confidence/confidence-{index}.png')
            plt.xlim([0, 50])
            plt.ylim([0, 50])
    ...
    if knn:
        ...
        if len(test_data) > 50:
            ...
            temp_mean, temp_confidence = predict_knn(features_to_predict, train_features, train_labels)

            plt.figure()
            plt.imshow(np.reshape(temp_mean, (50,50)), vmin=min_limit, vmax=max_limit, cmap='RdYlGn_r') # (1, 50, 50, 1)
            plt.colorbar()
            ...
            plt.figure()
            plt.imshow(np.reshape(temp_confidence, (50,50)), vmin=min_limit, vmax=max_limit, cmap='Reds') # (1, 50, 50, 1)
            plt.colorbar()
    ...
    if sensor:
        sensor_data_past = sensor_data[np.where((sensor_data[:,0] <= i) & (sensor_data[:,0] >= (i-5)))]

        plt.figure()
        if len(sensor_data_past) == 0:
            plt.scatter(sensor_data_past[:,1], sensor_data_past[:,2])
        else:
            pz = np.array((sensor_data_past[:,0]-np.min(sensor_data_past[:,0]))/(np.max(sensor_data_past[:,0])-np.min(sensor_data_past[:,0])))
            colors = np.array([[176/255, 10/255, 20/255, 1.        ]]*(len(sensor_data_past)))
            colors[:,-1] = pz
            plt.scatter(sensor_data_past[:,1], sensor_data_past[:,2], color=colors, marker='.')
        #plt.show()
    ...
    if gt:
        ...
        plt.figure()
        plt.imshow(np.reshape(true, (50,50)), vmin=min_limit, vmax=max_limit, cmap='RdYlGn_r') # (1, 50, 50, 1)
        plt.colorbar()
