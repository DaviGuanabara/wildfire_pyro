...
def generate_sensor_data():
    ...
    start_time = np.random.uniform(
        times[0], times[1] - lifetime)  # start at any time

    # A dataframe with data collected by the sensor
    df = pd.DataFrame({
        # perform the same number of measurement uniformly during lifespan
...
validation = data[(data['t'] >= 4 * math.pi) &
                  (data['t'] < 6 * math.pi)]  # 3rd day
